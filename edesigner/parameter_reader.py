...
class Parameters:
    def __init__(self, path, fsource='dict', how='to_list', multiple=False):
        ...
        try:
            ...
            lineas = f.readlines()
            f.close()
            row_lineas = copy.deepcopy([linea.rstrip('\r\n').split('\t') for linea in lineas if len(linea) > 1])
            lineas = []
            if fsource == 'dict':
                lineas = copy.deepcopy(row_lineas)
            elif fsource == 'list':
                for i in range(len(row_lineas)):  # to avoid having lines with less tabs than required
                    if len(row_lineas[i]) < len(row_lineas[0]):
                        row_lineas[i] += ['' for _ in range(len(row_lineas[0]) - len(row_lineas[i]))]
